...
const struct usb_device_id usb_drv_id_table[] = {
    {USB_DEVICE_INTERFACE_CLASS(0x0477, 0x1122, 0XFE)},
    {}
};
...
ssize_t usb_drv_read (struct file *file, char __user *buffer, size_t count, loff_t *offset) {
    ...
    __u8 endpoint = 0;
    __u8 request = START_SOUND_TRANSMIT;
    __u8 requesttype = 0xCC;// TYPE_VENDOR | RECEPIENT_DEVICE
    __u16 index = 0;
    __u16 value = 0;
...
static void usb_blk_callback(struct urb* urb) {
    if (urb->status == 0) {
        printk("Bulk received: %i bytes.\n", urb->actual_length);
    } else {
...
static void usb_int_callback(struct urb* urb) {
    ...
    // unsigned int pipe_blk;
    // int actual_length = 0;
    // pipe_blk = usb_rcvbulkpipe(usb_drv_device, 1);
...
int usb_drv_probe (struct usb_interface *intf, const struct usb_device_id *id) {
    ...
    unsigned int pipe;
    struct usb_host_endpoint *ep_blk;
    unsigned int pipe_blk;
    ...
    urb_blk = usb_alloc_urb(0, GFP_KERNEL);
    pipe = usb_rcvintpipe(usb_drv_device, 2);
    pipe_blk = usb_rcvbulkpipe(usb_drv_device, 1);
    ...
    usb_fill_bulk_urb(urb_blk, usb_drv_device, pipe_blk, urb_blk_buffer, BLK_SIZE, 
        usb_blk_callback, NULL);
    if (usb_submit_urb(urb, GFP_KERNEL)) {
        kfree(urb_buffer);
        usb_free_urb(urb);
...
void usb_drv_disconnect(struct usb_interface *intf) {
    ...
    kfree(urb_buffer);
    kfree(urb_blk_buffer);
    usb_free_urb(urb);
    usb_free_urb(urb_blk);
...
int __init usb_drv_init(void) {
    ...
    printk("Initializing\n");
    usb_buffer = kmalloc(16, GFP_KERNEL);
